...
def GetTrainTestLists(x, y, fold=5):
    ...
    for index in range(fold):
        ...
        if (index + 1) * test_data_length <= len(x):
            x_test = x[index *
                       test_data_length: (index + 1) * test_data_length]
            y_test = y[index *
                       test_data_length: (index + 1) * test_data_length]
            x_train = x[: index * test_data_length] + \
                x[(index + 1) * test_data_length:]
            y_train = y[: index * test_data_length] + \
                y[(index + 1) * test_data_length:]
        else:
...
def StackSvmBert():
    ...
    with open(data_dir + 'essays_sentences.json', encoding='utf-8') as f:
        ...
        y_essays = argNonArgMap([sent['sent-class']
                                for sent in essays_sentences])

    with open(data_dir + 'web_discourse.json', encoding='utf-8') as f:
        web_d_sentences = json.load(f)
        y_web_d = argNonArgMap([sent['sent-class']
                               for sent in web_d_sentences])
        web_d_sentences = ProcessRowSentences(
            [sent['sent-text'] for sent in web_d_sentences])
    ...
    bert_model, bert_preds = bert_train_and_predict(
        temp_text, temp_labels, test_text)
    svm_model, svm_preds = svm_train_and_predict(
        temp_text, temp_labels, test_text)

    meta_fold = 5
    x_train_list, x_test_list, y_train_list, y_test_list = GetTrainTestLists(
        temp_text, temp_labels, meta_fold)

    x_meta_train, y_meta_train = [], []
    for index in range(meta_fold):
        print(f'meta model data processing: fold {index + 1}')

        train_text, val_text, train_labels, val_labels = x_train_list[
            index], x_test_list[index], y_train_list[index], y_test_list[index]
        print(len(train_text), len(val_text),
              len(train_labels), len(val_labels))

        bert_model, y_bert = bert_train_and_predict(
            train_text, train_labels, val_text)
        svm_model, y_svm = svm_train_and_predict(
            train_text, train_labels, val_text)
        x_meta_train += [y_svm[idx][:1] + y_bert[idx]
    ...
    x_meta_test = [svm_preds[idx][:1] + bert_preds[idx]
    ...
    print(classification_report(test_labels, [
          int(pred[0] < pred[1]) for pred in svm_preds], digits=4))
    print(classification_report(test_labels, [
          int(pred[0] < pred[1]) for pred in bert_preds], digits=4))
    ...
    print(confusion_matrix(test_labels, [
          int(pred[0] < pred[1]) for pred in svm_preds]))
    print(confusion_matrix(test_labels, [
          int(pred[0] < pred[1]) for pred in bert_preds]))
